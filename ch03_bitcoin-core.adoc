...
+--disable-FEATURE+ flags, where pass:[`FEATURE`] is replaced by the
...
`--prefix=$HOME`::
  This overrides the default installation location (which is _/usr/local/_) for the resulting executable. Use `$HOME` to put everything in your home directory, or a different path.
