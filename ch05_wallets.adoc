...
* The first parameter to the PBKDF2 key-stretching function is the _entropy_ produced from step 6.

* The second parameter to the PBKDF2 key-stretching function is a _salt_. The salt is composed of the string constant "`mnemonic`" concatenated with an optional user-supplied passphrase string.
...
Tables <<bip39_128_no_pass>>, <<bip39_128_w_pass>>, and <<bip39_256_no_pass>> show some examples of recovery codes and the seeds ((("key-stretching functions", startref="key-stretch")))they produce.
...
separated by a slash (/) character (see <<table_4-8>>).
...
would be M/44++'++/0++'++/0++'++/0/2. See <<table_4-9>>.
